...
export default function ProjectsPage() {
    ...
    return (
        <div>
                            ...
                            totalPages > 0 ? (
                                <div className="flex w-full justify-center">
                                    <Pagination
                                        ...
                                        total={totalPages}
                                        onChange={(newPage) => setPagination({ ...pagination, page: newPage - 1 })}
                                    />
                                </div>
                            ) : null
                        }
                        ...
                        >
                            {(item) => (
                                ...
                                >
                                    ...
                                    <TableCell><ProjectStatusChip status={item.status} /></TableCell>
                                    <TableCell><ProjectTypeChip type={item.type} /></TableCell>
                                    <TableCell>{item.createdAt.toLocaleDateString()}</TableCell>
                                    <TableCell>{item.updatedAt.toLocaleDateString()}</TableCell>
                                    <TableCell>
                                        <Button size="sm" color="primary" isIconOnly onPress={() => router.push(`/internal/projects/${item.id}`)}>
            ...
            <CreateProjectForm isOpen={isCreateOpen} onClose={() => setIsCreateOpen(false)} />
        </div>
    );
}