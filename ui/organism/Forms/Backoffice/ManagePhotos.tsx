...
import { createProjectPhotoFrontendSchema, CreateProjectPhotoFrontendSchema } from "@/domain/ProjectPhoto";
    ...
    return (<div>
        <Section>
            <Container>
                <div className="flex justify-between items-center mb-4">
                    ...
                    <Button color="primary" onPress={() => setIsCreateOpen(true)}>Agregar foto</Button>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {project.photos.map((photo) => (
                        <div key={photo.id} className="relative rounded-lg overflow-hidden">
                            <img src={`https://storage.googleapis.com/${process.env.NEXT_PUBLIC_GOOGLE_STORAGE_BUCKET_NAME}/${getProdOrDevSuffix()}/${photo.url}`} alt={photo.url} />
                            <Button className="absolute top-2 right-2" color="danger" onPress={() => {
        ...
        <Modal isOpen={isCreateOpen} onClose={() => setIsCreateOpen(false)}>
            <ModalContent>
                ...
                <ModalFooter>
                    <Button onPress={() => formCreateFrontend.handleSubmit()} type="submit" color="primary" isLoading={createProjectPhotoMutation.isPending} isDisabled={createProjectPhotoMutation.isPending || formCreateFrontend.state.isSubmitting}>Agregar foto</Button>
                </ModalFooter>
            </ModalContent>
        </Modal>
