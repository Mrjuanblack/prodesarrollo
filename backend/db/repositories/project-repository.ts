...
import { CreateProject, Project, ProjectType, SimpleProject, UpdateProject } from "@/domain/Projects";
...
const errorHandler = new ErrorHandler_Repository(RepositoryErrorOrigin.PROJECTS);
...
export class ProjectRepository {
    public static async createProject(project: CreateProject): Promise<Project> {
        try {
            ...
            if (project.relatedProjects && project.relatedProjects.length > 0) {
                await db.insert(projectsToProjects).values(project.relatedProjects.map(projectId => ({
                ...
                finalRelatedProjects.push(...relatedProjects.map(relatedProject => ({
            ...
            return ProjectRepository.mapToDomain({
                ...
                photos: null,
                documents: null,
                relatedProjects: finalRelatedProjects.length > 0 ? finalRelatedProjects : null,
            });
        } catch (error) {
    ...
    public static async getProjectById(id: string): Promise<Project> {
        try {
            ...
            if (!result) {
                throw errorHandler.handleError(RepositoryErrorType.NOT_FOUND, new Error('Project not found'));
            }
    ...
    public static async getPaginatedProjects(pRequest: PaginationRequest, search?: string, year?: number, type?: ProjectType): Promise<PaginationResponse<Project>> {
        try {
            ...
            return {
                data: result.map(project => {
                    ...
                    return ProjectRepository.mapToDomain({
                        ...project,
                        photos: project.photos ? project.photos.map(photo => ({
                            id: photo.id,
    ...
    public static async getYearsWithProjects(): Promise<number[]> {
        try {
            const result = await db.select({
                year: sql<number>`EXTRACT(YEAR FROM ${projects.date})::integer`.as('year')
            })
            .from(projects)
    ...
    public static async updateProject(id: string, project: UpdateProject): Promise<Project> {
        try {
            ...
            await db.delete(projectsToProjects).where(or(eq(projectsToProjects.projectOneId, id), eq(projectsToProjects.projectTwoId, id)));
    ...
    public static mapToDomain(project: typeof projects.$inferSelect & { photos: ProjectPhoto[] | null, relatedProjects: SimpleProject[] | null } & { documents: ProjectDocument[] | null }): Project {
        return {
            id: project.id,
            title: project.title,
            description: project.description,
